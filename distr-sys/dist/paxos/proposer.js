...
class Proposer {
    constructor(nodeId, paxosState, httpClient, logger) {
        this.nodeId = nodeId;
     ...
     */
    async runConsensus(proposedValue, retryCount = 0) {
        const MAX_RETRIES = 3;
        ...
        this.logger.paxos('PROPOSE', `Starting proposal round (attempt ${retryCount + 1}/${MAX_RETRIES})`, { proposalNumber, proposedValue });
        // ──────────────────────────────────────────────────────────
        // PHASE 1: PREPARE
        // ──────────────────────────────────────────────────────────
        ...
        const alreadyDecided = prepareResponses.find((r) => 'alreadyDecided' in r.response && r.response.alreadyDecided === true);
        if (alreadyDecided) {
            const decidedValue = alreadyDecided.response.decidedValue;
            this.logger.paxos('PHASE-1', `Peer already decided on value ${decidedValue} — adopting decision`);
            ...
            (0, functions_1.executeDecision)(decidedValue, this.logger);
            return;
        }
        // Count successful promises (include our own implicit promise)
        const promiseCount = prepareResponses.filter((r) => r.response.promised).length + 1; // +1 for self
        this.logger.paxos('PHASE-1', `Received ${promiseCount} promises (need ${MAJORITY} for majority)`);
        if (promiseCount < MAJORITY) {
            this.logger.paxos('PHASE-1', `Failed to get majority promises (${promiseCount}/${MAJORITY}). Retrying after delay...`);
        ...
        for (const { response } of prepareResponses) {
            if (response.promised) {
                ...
                if (okResponse.acceptedProposal !== null &&
                    ...
                    this.logger.paxos('PHASE-1', `Adopting previously accepted value ${valueToPropose} from proposal ${highestAcceptedProposal}`);
                }
            }
        }
        ...
        const acceptResponses = await this.httpClient.sendAccept(proposalNumber, valueToPropose);
        // Check again if any peer decided in the meantime
        const decidedDuringAccept = acceptResponses.find((r) => 'alreadyDecided' in r.response && r.response.alreadyDecided === true);
        if (decidedDuringAccept) {
            const decidedValue = decidedDuringAccept.response.decidedValue;
            this.logger.paxos('PHASE-2', `Peer already decided on value ${decidedValue} — adopting`);
            this.paxosState.decide(decidedValue);
            this.logger.consensus(decidedValue);
            (0, functions_1.executeDecision)(decidedValue, this.logger);
            return;
        ...
        const acceptedCount = acceptResponses.filter((r) => r.response.accepted).length + 1; // +1 for self
        this.logger.paxos('PHASE-2', `Received ${acceptedCount} acceptances (need ${MAJORITY} for majority)`);
        if (acceptedCount < MAJORITY) {
            this.logger.paxos('PHASE-2', `Failed to get majority acceptances (${acceptedCount}/${MAJORITY}). Retrying after delay...`);
            const backoff = 2000 + Math.random() * 3000;
            await this.sleep(backoff);
            return this.runConsensus(proposedValue, retryCount + 1);
        }
        ...
        // CONSENSUS REACHED!
        // ──────────────────────────────────────────────────────────
        this.logger.paxos('PHASE-2', `★ Majority achieved! Value ${valueToPropose} is CHOSEN ★`);
        // Record decision locally
        this.paxosState.decide(valueToPropose);
        this.logger.consensus(valueToPropose);
        // Execute the function locally
        (0, functions_1.executeDecision)(valueToPropose, this.logger);
