...
class HttpClient {
    constructor(nodeId, nodeList, logger) {
        this.nodeId = nodeId;
        this.logger = logger;
    ...
    async queryAllStatuses() {
        ...
        const requests = this.peers.map(async (peerUrl) => {
            try {
                const res = await axios_1.default.get(`${peerUrl}/status`, {
                    timeout: TIMEOUT_MS,
                });
     ...
     */
    async sendPrepare(proposalNumber) {
        this.logger.paxos('PREPARE', `Sending PREPARE(${proposalNumber}) to all peers`);
        const promises = this.peers.map((peerUrl) => axios_1.default
            .post(`${peerUrl}/paxos/prepare`, { proposalNumber, proposerId: this.nodeId }, { timeout: TIMEOUT_MS })
            .then((res) => {
     ...
     * Returns an array of { peer, response } for those that replied.
     */
    async sendAccept(proposalNumber, value) {
        this.logger.paxos('ACCEPT', `Sending ACCEPT(${proposalNumber}, value=${value}) to all peers`);
        const promises = this.peers.map((peerUrl) => axios_1.default
            .post(`${peerUrl}/paxos/accept`, { proposalNumber, value, proposerId: this.nodeId }, { timeout: TIMEOUT_MS })
            .then((res) => {
     ...
     */
    async broadcastDecision(value, proposalNumber) {
        this.logger.paxos('DECIDE', `Broadcasting decision: value=${value}, proposal=${proposalNumber}`);
        const promises = this.peers.map((peerUrl) => axios_1.default
            .post(`${peerUrl}/paxos/decide`, { value, proposalNumber, deciderId: this.nodeId }, { timeout: TIMEOUT_MS })
            .catch((err) => {
