...
function createRoutes(paxosState, logger) {
    ...
    router.post('/paxos/prepare', (req, res) => {
        const { proposalNumber, proposerId } = req.body;
        logger.paxos('PREPARE', `Received PREPARE(${proposalNumber}) from Node ${proposerId}`);
        // If we've already decided, tell the proposer immediately
        if (paxosState.hasDecided()) {
            logger.paxos('PREPARE', `Already decided value ${paxosState.decidedValue} — sending NACK with decision`);
            res.json({
                promised: false,
                alreadyDecided: true,
        ...
        if (result.promised) {
            ...
            logger.paxos('PROMISE', `Rejecting PREPARE(${proposalNumber}) — already promised to higher`);
        }
        res.json(result);
    });
    // ─── POST /paxos/accept ────────────────────────────────────────
    ...
    //   Otherwise → REJECT
    router.post('/paxos/accept', (req, res) => {
        const { proposalNumber, value, proposerId } = req.body;
        logger.paxos('ACCEPT', `Received ACCEPT(${proposalNumber}, value=${value}) from Node ${proposerId}`);
        // If we've already decided, inform the proposer
        if (paxosState.hasDecided()) {
            logger.paxos('ACCEPT', `Already decided value ${paxosState.decidedValue} — rejecting`);
            res.json({
                accepted: false,
                alreadyDecided: true,
                decidedValue: paxosState.decidedValue,
        ...
        if (result.accepted) {
            ...
            logger.paxos('ACCEPTED', `Rejected ACCEPT(${proposalNumber}) — already promised to higher`);
        }
        res.json(result);
    });
    // ─── POST /paxos/decide ────────────────────────────────────────
    ...
    router.post('/paxos/decide', (req, res) => {
        const { value, proposalNumber, deciderId } = req.body;
        logger.paxos('DECIDE', `Received DECIDE from Node ${deciderId}: value=${value}, proposal=${proposalNumber}`);
        if (!paxosState.hasDecided()) {
